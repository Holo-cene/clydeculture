...
Project context for Codex. Read this before doing anything in this repo.
...
source contracts, Codex should still run a consistency check where possible.

### Step 1 — test first, no production code

Codex must first write or update the relevant test(s), then stop.
...
**Codex must not implement production code in this step.**
...
Only after the user gives the exact follow-up instruction, Codex may implement production code.
...
Before writing or editing tests, Codex must identify the smallest relevant test set for the
...
Codex should prefer specific targeted tests first, then broader regression checks after
implementation.
...
If a test cannot be written because the contract is missing, Codex must stop and propose
...
**To authenticate:** run `Codex /mcp` from a terminal, select `supabase`, choose `Authenticate`, complete the browser flow. Do not commit credentials or paste tokens into any file.
...
Codex may use it to: inspect table definitions, check RLS policies, review functions/triggers/constraints, and support SQL assertion writing.
