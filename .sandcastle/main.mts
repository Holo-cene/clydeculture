...
// This repo is a pnpm workspace, so the host's node_modules is a tree of
// symlinks into a content-addressed store with darwin-arm64 native binaries —
// not safe to copy into a Linux container. We install cleanly inside instead.
// To speed this up later, mount the host pnpm store read-only via
// docker({ mounts: [{ hostPath: "~/.local/share/pnpm/store", sandboxPath: "/home/agent/.pnpm-store", readonly: true }] }).
